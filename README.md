...
- `Integer` - целое число (в JSON-обьекте должно являться именно числом, а не строкой с числом)
- `Float` - дробное число
...
**ВАЖНО! Созданные параметры невозможно удалить из памяти. Если одни и те же параметры требуется использовать несколько раз, то сохраняйте их куда-нибудь или кешируйте через static-переменные**
